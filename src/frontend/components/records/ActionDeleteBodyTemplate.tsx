...
const ActionDeleteBodyTemplate = ({ stockId }: { stockId: string }) => {
  const { deleteSelectedDetail } = useActionDeleteButton(stockId)
  return (
    <React.Fragment>
      <Button icon="pi pi-trash" className="p-button-rounded p-button-warning mr-2" onClick={() => deleteSelectedDetail()}/>
    </React.Fragment>
  )
}
